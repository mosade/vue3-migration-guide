...
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 组件的 props、attrs、slots、事件透传与原调用方兼容。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- Vue 2 functional context 中的 `data`、`listeners`、`scopedSlots`、`children` 不能原样照搬；要重新确认 attrs、slots、emits 的边界。
...
原搜索线索：全局搜索 `functional: true`、`<template functional>`、`context.props`、`context.children`。

```bash
rg -n 'functional\s*:\s*true|<template\s+functional|context\.props|context\.children|functional' src
```
...
- 简单展示组件改为函数组件；需要生命周期、状态或复杂透传的组件改为普通组件。
- 不要原样照搬 Vue2 functional context 的 `data`、`listeners`、`scopedSlots`、`children`。
- 对“必改”命中做最小兼容性修改。
