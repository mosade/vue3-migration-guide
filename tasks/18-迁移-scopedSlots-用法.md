...
- 迁移运行时代码中的 `$scopedSlots` 访问。
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 有无插槽、默认内容 fallback、作用域参数传递三类场景都正确。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- Vue 3 中 slot 统一为函数，判断存在性和调用方式都要改；不要把 `$slots.name` 当成静态 VNode 数组直接拼接。
...
原搜索线索：全局搜索 `$scopedSlots`、`scopedSlots`、`this.$slots`、`context.scopedSlots`。

```bash
rg -n '\$scopedSlots|scopedSlots|this\.\$slots|context\.scopedSlots|\$slots' src
```
...
- 将 `$scopedSlots.xxx` 合并到 `$slots.xxx` 的函数调用模型；render 函数中按 Vue 3 slot 函数返回 VNode 数组处理。
- 将 `$scopedSlots.xxx` 合并到 `$slots.xxx` 的函数调用模型。
- slot 存在性判断和 fallback 必须按 Vue3 slot 函数语义处理。
- 对“必改”命中做最小兼容性修改。
...
- 有无插槽、默认 fallback、作用域参数传递都正确。
- render 函数没有把 slot 函数误当静态 VNode 数组。
