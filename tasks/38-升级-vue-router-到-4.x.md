# 38.升级 vue-router 到 4.x

## 目标

- 将路由依赖升级到 Vue 3 对应主版本。
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 依赖解析到 Vue Router 4，项目中不再引用 Vue Router 3 专属安装方式。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 依赖安装成功只是起点，必须继续完成创建方式、history、catch-all、ready、router-view 组合任务。
...
原搜索线索：检查 `package.json` 中 `vue-router`、`vue-router/types`、路由相关插件依赖。

```bash
rg -n 'vue-router|vue-router/types|router' package.json package-lock.json yarn.lock pnpm-lock.yaml 2>/dev/null
```
...
- 升级到 Vue Router 4，并同步处理类型、构建别名和插件中对旧 router API 的引用。
- 本任务不负责完整改写路由表，创建方式、history、catch-all 等由后续任务处理。
- 对“必改”命中做最小兼容性修改。
