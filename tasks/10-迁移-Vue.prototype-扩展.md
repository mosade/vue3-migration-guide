...
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 组件内原有 `this.$xxx` 能访问，且入口初始化顺序保证使用前已注册。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 只迁移真正的全局实例属性；工具函数也可以直接 import，避免继续扩大隐式全局依赖。
...
原搜索线索：全局搜索 `Vue.prototype`、`this.$` 前缀的全局工具访问。

```bash
rg -n 'Vue\.prototype|app\.config\.globalProperties|this\.\$[A-Za-z_][A-Za-z0-9_]*' src
```
...
- 将 `Vue.prototype.$xxx = value` 改为 `app.config.globalProperties.$xxx = value`。
- 普通工具函数优先保持原调用方式，不借本任务扩大隐式全局依赖。
- 对“必改”命中做最小兼容性修改。
