...
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 原全局组件在模板中可继续解析，无 unknown component 警告。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 自动批量注册如果依赖 webpack `require.context`，要确认构建链仍支持；组件名大小写和前缀规则不要顺手改动。
...
原搜索线索：全局搜索 `Vue.component(`、批量注册组件的 require/import 逻辑。

```bash
rg -n 'Vue\.component\(|app\.component\(|require\.context|components/index|global.*component' src
```
...
- 改为在应用创建后调用 `app.component(name, component)`。
- 将 `Vue.component(name, component)` 改为 `app.component(name, component)`。
- 批量注册逻辑只改注册 API，不改组件名规则、前缀和目录结构。
- 对“必改”命中做最小兼容性修改。
...
- 原全局组件在模板中可继续解析。
- 浏览器控制台没有由本任务引入的 unknown component 警告。
