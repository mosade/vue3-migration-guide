...
- 修复同节点 `v-if` 与 `v-for` 的优先级变化问题。
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 空列表、部分过滤、全量展示三种状态与原业务预期一致。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- Vue 3 中 `v-if` 优先级高于 `v-for`，原来依赖循环变量的条件可能会直接失效；不要为了快速修复引入额外无意义 DOM。
...
原搜索线索：全局搜索同时包含 `v-if` 和 `v-for` 的模板节点。

```bash
rg -n 'v-if=.*v-for|v-for=.*v-if' src
```
...
- 如果是过滤列表，优先改成计算属性；如果是整段条件渲染，将 `v-if` 放到外层容器或 `<template>`。
- 过滤列表优先改成计算属性；整段条件渲染把 `v-if` 放到外层容器或 `<template>`。
- 避免为了快速修复引入影响布局的额外 DOM。
- 对“必改”命中做最小兼容性修改。
