...
- 不因为代码看起来旧、乱、重复就顺手重构。
- Vue 3 未声明到 `emits` 的监听可能作为原生监听透传到根节点，多根组件或包装组件必须明确事件落点。
...
```bash
rg -n '\.native\b|emits\s*:|inheritAttrs|\$attrs' src
```
...
- 如果监听的是组件内部根元素事件，改为子组件显式 `$emit` 并声明 `emits`；如果只是透传原生事件，确认 `$attrs` 会落到正确元素。
- 移除 `.native`；必要时由子组件显式 `$emit` 并声明 `emits`。
- 只在确认事件落点时透传 `$attrs`，多根组件不自动处理。
- 对“必改”命中做最小兼容性修改。
