...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 只看终端成功不够，必须打开页面确认没有启动即白屏的运行时错误。
...
- 实际启动 `vue-cli-service serve`，确认编译完成、HMR 不持续报错、浏览器可访问。
- 实际启动 `vue-cli-service serve` 或项目等价脚本。
- 终端成功后还要打开页面确认没有启动即白屏。
- 对“必改”命中做最小兼容性修改。
