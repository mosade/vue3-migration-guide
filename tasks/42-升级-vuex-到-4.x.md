...
- 将状态管理依赖升级到 Vue 3 对应主版本。
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 依赖解析到 Vuex 4，store 创建和插件初始化无旧版 Vue 安装依赖。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- Vuex 4 API 变化不大，但安装方式、类型、插件里访问 Vue 实例的逻辑可能会坏。
...
原搜索线索：检查 `package.json` 中 `vuex`、Vuex 插件、中间件、持久化插件。

```bash
rg -n 'vuex|vuex-persist|vuex-router-sync|store' package.json package-lock.json yarn.lock pnpm-lock.yaml 2>/dev/null
```
...
- 升级到 Vuex 4，并确认配套插件支持 Vue 3/Vuex 4。
- 升级到 Vuex 4，并确认配套插件支持 Vue3/Vuex4。
- 保留原 store 结构，不借升级迁移到 Pinia。
- 对“必改”命中做最小兼容性修改。
