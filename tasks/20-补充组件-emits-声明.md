...
- 为事件输出组件补充 Vue 3 事件声明。
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 控制台没有未知事件/重复触发问题，父组件监听的自定义事件行为稳定。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- Vue 3 会用 `emits` 判断事件是否应从 `$attrs` 中剔除，漏声明会导致事件可能被透传到根元素并重复触发。
...
原搜索线索：全局搜索 `$emit(`、`emits:`、`defineEmits`、`@` 监听自定义组件。

```bash
rg -n '\$emit\(|emits\s*:|defineEmits|@[A-Za-z0-9_:-]+=|v-on:' src
```
...
- 为组件声明所有对外事件；事件只是内部 DOM 监听时不要放进 `emits`；事件有 payload 约束时可以补 validator。
- 为组件声明所有对外事件；内部 DOM 监听事件不要放进 `emits`。
- payload validator 只在已有约束清晰时补充，不顺手设计新事件契约。
- 对“必改”命中做最小兼容性修改。
