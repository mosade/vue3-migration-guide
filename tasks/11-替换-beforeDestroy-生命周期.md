...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 这个钩子常用于解绑事件、停止定时器、取消请求；迁移时顺手确认清理函数仍能拿到对应引用。
...
原搜索线索：全局搜索 `beforeDestroy`。

```bash
rg -n '\bbeforeDestroy\b|\bbeforeUnmount\b' src
```
...
- 将 Options API 中的 `beforeDestroy` 改为 `beforeUnmount`。
- Options API 中的 `beforeDestroy` 改为 `beforeUnmount`。
- 只检查与清理逻辑相关的引用，不重写清理实现。
- 对“必改”命中做最小兼容性修改。
