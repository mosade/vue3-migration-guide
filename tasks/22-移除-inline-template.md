...
- 删除不再推荐的 `inline-template` 用法。
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 模板变量来源清晰，组件渲染结果与原页面一致。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- inline-template 里的变量解析边界容易混淆，迁移时要确认每个变量来自父作用域还是子组件实例。
...
原搜索线索：全局搜索 `inline-template`。

```bash
rg -n 'inline-template' src
```
...
- 把内联模板迁回子组件自己的 `<template>`、render 函数或显式 slot；父组件只负责传 props/slots。
- 把内联模板迁回子组件自己的 `<template>`、render 函数或显式 slot。
- 迁移前确认变量来自父作用域还是子组件实例。
- 对“必改”命中做最小兼容性修改。
