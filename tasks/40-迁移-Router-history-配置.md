...
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 刷新页面、直接访问深层路由、hash/history 模式下的跳转都正常。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- `base` 不再是 router 顶层配置，生产部署路径和 nginx/history fallback 要一起确认。
...
原搜索线索：全局搜索 `mode:`、`base:`、`history:`、`hash`、`abstract`。

```bash
rg -n 'mode\s*:|base\s*:|history\s*:|createWebHistory|createWebHashHistory|createMemoryHistory|hash|abstract' src vue.config.js 2>/dev/null
```
...
- `mode: 'history'` 改为 `createWebHistory(base)`，`mode: 'hash'` 改为 `createWebHashHistory(base)`；测试环境或非浏览器环境按需使用 memory history。
- `mode: 'history'` 改为 `createWebHistory(base)`，`mode: 'hash'` 改为 `createWebHashHistory(base)`。
- `base` 不再是 router 顶层配置，部署路径要单独验收。
- 对“必改”命中做最小兼容性修改。
...
- 刷新页面、直接访问深层路由、hash/history 模式下跳转都正常。
- 生产部署路径和 history fallback 已确认。
