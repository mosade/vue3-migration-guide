...
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 组件内 `this.$store`、map helpers、动态模块、插件逻辑都能正常工作。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 保留原 store 模块结构，先不要顺手重构状态管理；插件、命名空间模块、动态模块注册需要验证。
...
原搜索线索：全局搜索 `Vue.use(Vuex)`、`new Vuex.Store`、`createStore`、`mapState`。

```bash
rg -n 'Vue\.use\(Vuex\)|new Vuex\.Store|createStore|mapState|mapGetters|mapActions|mapMutations|app\.use\(store\)' src
```
...
- 使用 `createStore` 创建 store，并在入口中通过 `app.use(store)` 注册。
- 保留原模块、插件、命名空间和动态模块注册方式。
- 对“必改”命中做最小兼容性修改。
