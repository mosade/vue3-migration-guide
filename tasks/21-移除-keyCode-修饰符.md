...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 不同浏览器/输入法下 `event.key` 可能有差异，快捷键和表单提交场景要实测。
...
原搜索线索：全局搜索 `@keyup.`、`@keydown.`、`config.keyCodes`、数字键码如 `.13`、`.27`。

```bash
rg -n '@keyup\.[0-9]+|@keydown\.[0-9]+|v-on:keyup\.[0-9]+|v-on:keydown\.[0-9]+|config\.keyCodes|keyCodes|\.(13|27|9|37|38|39|40)\b' src
```
...
- 将 keyCode 修饰符改为键名修饰符，例如 `.enter`、`.esc`、`.tab`，复杂组合改为事件处理函数内判断 `event.key`。
- 数字 keyCode 修饰符改为键名修饰符，例如 `.enter`、`.esc`、`.tab`。
- 复杂组合改为事件处理函数内判断 `event.key`。
- 对“必改”命中做最小兼容性修改。
