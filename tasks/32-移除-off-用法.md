...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 匿名 handler 无法精准解绑，迁移时应抽成稳定函数引用或保存 unsubscribe。
...
原搜索线索：全局搜索 `.$off(`、`$off(`，并对照同名 `$on`。

```bash
rg -n '\.\$off\(|\$off\(|\.\$on\(|\$on\(' src
```
...
- 使用新 emitter 的取消订阅 API；如果新 API 返回 unsubscribe 函数，统一在组件卸载时调用。
- 使用新 emitter 的取消订阅 API，或保存并调用 unsubscribe 函数。
- 匿名 handler 无法精准解绑时停止并报告。
- 对“必改”命中做最小兼容性修改。
...
- 页面卸载、弹窗关闭、组件销毁后不再响应旧事件。
- 取消订阅使用稳定 handler 或 unsubscribe。
