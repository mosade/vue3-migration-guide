...
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 组件协作不依赖内部渲染顺序，增删子组件或调整模板结构后行为仍稳定。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 不要用 DOM 查询替代 `$children`；`ref` 在 `v-for`、条件渲染、异步组件下的时机和结构都要单独验证。
...
原搜索线索：全局搜索 `$children`、`this.$parent`、按下标访问子组件的逻辑。

```bash
rg -n '\$children|this\.\$parent|\$parent|children\[[0-9]+\]' src
```
...
- 单个确定子组件用 `ref`；跨层级协作用 provide/inject；列表型子组件用数据驱动或显式注册/注销机制。
- 不要用 DOM 查询替代 `$children`。
- 对“必改”命中做最小兼容性修改。
