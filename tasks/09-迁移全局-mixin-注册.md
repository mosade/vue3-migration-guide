...
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 全局混入仍生效，且没有引入全局重复请求、重复埋点或生命周期报错。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 全局 mixin 会影响所有组件，迁移后重点确认生命周期命名、实例属性访问和重复执行问题。
...
原搜索线索：全局搜索 `Vue.mixin(`、全局混入文件、埋点/权限/字典类 mixin。

```bash
rg -n 'Vue\.mixin\(|app\.mixin\(|mixin' src
```
...
- 改为 `app.mixin(mixin)`。
- 将 `Vue.mixin(mixin)` 改为 `app.mixin(mixin)`。
- 全局 mixin 影响面大，发现生命周期或实例属性耦合时只处理本任务直接阻塞。
- 对“必改”命中做最小兼容性修改。
