...
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 数组 push/splice、索引项属性修改、整体替换都按业务需要触发，且没有明显性能退化。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 不要默认给所有数组 watch 加 deep，大型列表会有性能风险；能通过明确事件、提交点或计算属性解决时优先减少深度监听。
...
原搜索线索：全局搜索 `watch:`、`$watch(`、`watch(`，重点看监听数组、对象列表、表格数据、树数据的逻辑。

```bash
rg -n 'watch\s*:|\$watch\(|\bwatch\s*\(' src
```
...
- 需要监听数组内部增删改时补 `deep: true` 或改为监听派生值；只关心整体替换时保持浅监听。
- 需要监听数组内部增删改时补 `deep: true` 或改为监听派生值。
- 不要默认给所有数组 watch 加 deep，大型列表优先记录风险。
- 对“必改”命中做最小兼容性修改。
...
- 数组 push/splice、索引项属性修改、整体替换按业务需要触发。
- 大型列表没有明显性能退化。
