...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 如果使用 CSS module、scoped style 或第三方动画库，类名生成和作用域选择器要一起验证。
...
原搜索线索：全局搜索 `*-enter`、`*-leave`、`v-enter`、`v-leave`、`<transition`。

```bash
rg -n '<transition|[A-Za-z0-9_-]+-enter\b|[A-Za-z0-9_-]+-leave\b|v-enter\b|v-leave\b|enter-active|leave-active' src
```
...
- 将旧的 `*-enter` / `*-leave` 起始类名迁到 Vue 3 的 `*-enter-from` / `*-leave-from` 规则。
- 旧的 `*-enter` / `*-leave` 起始类名迁到 `*-enter-from` / `*-leave-from`。
- CSS module、scoped style、第三方动画库命中时同时验证类名生成和选择器作用域。
- 对“必改”命中做最小兼容性修改。
...
- 进入、离开、appear、路由切换动画都按预期触发。
- scoped style 或 CSS module 下类名仍匹配。
