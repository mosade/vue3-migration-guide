...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 一次性订阅也要处理组件提前卸载的清理，否则事件迟到时仍可能触发已销毁页面逻辑。
...
- 使用新 emitter 的 once 能力；如果没有 once，则用包装函数在第一次触发后主动取消订阅。
- 使用新 emitter 的 once 能力；没有 once 时用包装函数首触发后主动取消订阅。
- 一次性订阅也要处理组件提前卸载的清理。
- 对“必改”命中做最小兼容性修改。
