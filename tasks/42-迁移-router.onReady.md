...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 应用挂载顺序会影响首屏守卫、异步路由和埋点初始化；不要把依赖当前路由的逻辑提前到 ready 之前执行。
...
原搜索线索：全局搜索 `router.onReady`、`onReady(`。

```bash
rg -n 'router\.onReady|onReady\(|router\.isReady|isReady\(' src
```
...
- 将 callback 形式改为 `router.isReady().then(...)` 或入口启动前 `await router.isReady()`。
- callback 形式改为 `router.isReady().then(...)` 或入口启动前 `await router.isReady()`。
- 不要把依赖当前路由的逻辑提前到 ready 之前执行。
- 对“必改”命中做最小兼容性修改。
