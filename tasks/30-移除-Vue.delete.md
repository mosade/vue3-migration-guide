...
- 删除 Vue 2 响应式补丁 API 之一。
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 字段删除、列表删除后视图和派生状态都正确更新。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 删除数组元素不要用 `delete arr[index]`，应使用 `splice` 或过滤生成新数组。
...
原搜索线索：全局搜索 `Vue.delete`、`this.$delete`。

```bash
rg -n 'Vue\.delete|this\.\$delete|\$delete\(' src
```
...
- 普通删除用 `delete obj[key]`；需要保留不可变语义时通过解构或重建对象删除字段。
- 普通删除用 `delete obj[key]`；数组元素删除用 `splice` 或过滤生成新数组。
- 不要用 `delete arr[index]` 制造数组空洞。
- 对“必改”命中做最小兼容性修改。
