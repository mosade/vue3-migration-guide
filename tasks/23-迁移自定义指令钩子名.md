...
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 指令初始化、更新、卸载都在预期时机执行，且没有遗留事件监听或定时器。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 如果指令依赖组件实例，Vue 3 的 `binding.instance` 行为要单独确认；如果指令操作 DOM 或注册全局事件，必须检查清理时机。
...
原搜索线索：全局搜索 `Vue.directive`、`directives:`、`bind(`、`inserted(`、`componentUpdated(`、`unbind(`。

```bash
rg -n 'Vue\.directive|directives\s*:|\bbind\s*\(|\binserted\s*\(|componentUpdated\s*\(|\bunbind\s*\(|beforeMount\s*\(|mounted\s*\(|updated\s*\(|unmounted\s*\(' src
```
...
- `bind` 改为 `beforeMount`，`inserted` 改为 `mounted`，`componentUpdated` 改为 `updated`，`unbind` 改为 `unmounted`。
- 指令操作 DOM、注册全局事件或依赖组件实例时必须验证清理时机。
- 对“必改”命中做最小兼容性修改。
