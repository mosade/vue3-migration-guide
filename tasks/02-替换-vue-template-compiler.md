...
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 模板编译链使用 Vue 3 SFC compiler，安装和构建过程中不再报 Vue/compiler 版本不匹配。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 不要同时保留 Vue 2 和 Vue 3 的 SFC 编译器；老 loader 或测试工具可能会间接拉回旧编译器。
...
原搜索线索：检查 `package.json`、lockfile、构建配置中是否仍引用 `vue-template-compiler`。

```bash
rg -n 'vue-template-compiler|@vue/compiler-sfc|vue-loader' package.json package-lock.json yarn.lock pnpm-lock.yaml vue.config.js babel.config.js jest.config.js 2>/dev/null
```
...
- 删除 `vue-template-compiler`，改用 `@vue/compiler-sfc`，并保持其版本与 `vue` 主版本一致。
- 删除 Vue2 专用 `vue-template-compiler`，使用与 `vue` 主版本一致的 `@vue/compiler-sfc`。
- 不要同时保留 Vue2 和 Vue3 的 SFC 编译器作为有效依赖。
- 对“必改”命中做最小兼容性修改。
...
- 安装和构建过程中不再出现 Vue/compiler 版本不匹配。
- 代表性 `.vue` 文件可以进入 Vue 3 SFC 编译链。
