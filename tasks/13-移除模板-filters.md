...
- 移除 Vue 3 已删除的模板过滤器能力。
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 模板中不再存在 Vue filter 语法，格式化结果与原页面一致。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 不要把表达式里的管道字符误判；全局 filter 要先落到统一工具模块，避免每个组件复制一份实现。
...
原搜索线索：全局搜索模板中的 `|`，以及组件选项里的 `filters:`。

```bash
rg -n 'filters\s*:|\|\s*[A-Za-z_$][A-Za-z0-9_$]*' src
```
...
- 纯格式化逻辑改为普通工具函数；依赖响应式状态的逻辑改为 computed；需要传参的过滤器改为显式函数调用。
- 纯格式化 filter 改为普通工具函数；依赖响应式状态的逻辑改为 computed。
- 不要把 JS 表达式、CSS、文案中的管道字符误判为 Vue filter。
- 对“必改”命中做最小兼容性修改。
