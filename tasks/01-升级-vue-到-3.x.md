...
- 将 `vue` 主依赖升级到 Vue 3 正式版本。
- 只处理本任务范围内的问题。
- 默认保持现有业务行为和组件外部 API。
- 依赖树中主运行时为 Vue 3，且没有继续安装 Vue 2 专用编译器作为有效依赖。
...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 依赖安装成功不代表迁移完成，后续任务会继续处理编译链、入口、router、store 和运行时代码。
...
原搜索线索：检查 `package.json` 中 `vue`、`vue-template-compiler`、`vue-loader`、`vue-router`、`vuex`、UI 组件库及 Vue 插件。

```bash
rg -n 'vue|vue-template-compiler|@vue/compiler-sfc|vue-loader|vue-router|vuex|element-ui|ant-design-vue|iview|view-design' package.json package-lock.json yarn.lock pnpm-lock.yaml 2>/dev/null
```
...
- 直接升级到 Vue 3，不引入 `@vue/compat`；同步清理与 Vue 2 强绑定的 peer/version 约束。
- 只修改依赖声明和锁文件中与 Vue3 运行时直接相关的条目。
- 发现 UI 组件库或私有插件不支持 Vue3 时记录为阻塞，不通过 `@vue/compat` 绕过。
- 对“必改”命中做最小兼容性修改。
