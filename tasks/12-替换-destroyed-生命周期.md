...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 如果同一组件同时存在 `beforeDestroy` 和 `destroyed`，要保持原清理顺序语义。
...
原搜索线索：全局搜索 `destroyed`。

```bash
rg -n '\bdestroyed\b|\bunmounted\b' src
```
...
- 将 Options API 中的 `destroyed` 改为 `unmounted`。
- Options API 中的 `destroyed` 改为 `unmounted`。
- 同组件同时存在卸载前后钩子时保持原执行顺序语义。
- 对“必改”命中做最小兼容性修改。
