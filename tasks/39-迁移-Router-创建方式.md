...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 路由实例导出方式尽量保持兼容调用方；动态路由、权限守卫、重置路由逻辑要一起检查。
...
原搜索线索：全局搜索 `new Router`、`VueRouter`、`Vue.use(Router)`、`routes:`。

```bash
rg -n 'new Router|VueRouter|Vue\.use\(Router\)|Vue\.use\(VueRouter\)|routes\s*:|createRouter|app\.use\(router\)' src
```
...
- 使用 `createRouter({ history, routes })` 创建实例；删除 `Vue.use(Router)`；在入口中通过 `app.use(router)` 安装。
- 使用 `createRouter({ history, routes })` 创建实例。
- 删除 `Vue.use(Router)`，入口中通过 `app.use(router)` 安装。
- 对“必改”命中做最小兼容性修改。
