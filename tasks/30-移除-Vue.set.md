...
- 不因为代码看起来旧、乱、重复就顺手重构。
- 不要为了“保持响应式”保留多余包装；但 Vuex、表单缓存、深层对象合并处要确认引用变化是否被业务代码依赖。
...
原搜索线索：全局搜索 `Vue.set`、`this.$set`。

```bash
rg -n 'Vue\.set|this\.\$set|\$set\(' src
```
...
- 普通对象和数组优先改为直接赋值；需要触发不可变更新链路的地方用对象/数组替换。
- 普通对象和数组优先改为直接赋值。
- 需要不可变更新链路时用对象或数组替换，但不顺手重写状态结构。
- 对“必改”命中做最小兼容性修改。
